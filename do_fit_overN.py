...
try:
    with open('input/input_overN.yaml', 'r') as stream:
...
if uls:
    ...
    hist_prefix = 'ppmm'
    hist2_prefix = 'pm'
    plotname_prefix = 'bec_3d_fit_uls_overN'
...
if mult and not kt:
    ...
    for n1, n2 in ns:
        hist_suffix = '_qosl_g_n_%i_%i' % (n1, n2)
        plotname = plotname_prefix + '_mult_%i_%i' % (n1, n2) + plotname_suffix
        comment = comment_prefix + 'n_{obs} #geq %i' % (n1)
...
if kt and not mult:
    ...
    add_csv_line(plotname_prefix)
    for kt1, kt2 in kts:
        hist_suffix = '_qosl_g_kt_%i_%i' % (kt1, kt2)
        plotname = plotname_prefix + '_kt_%i_%i' % (kt1, kt2) + plotname_suffix
...
if mult and kt:
    for n1, n2 in ns:
        for kt1, kt2 in kts:
            hist_suffix = '_qosl_g_kt_%i_%i_n_%i_%i' % (kt1, kt2, n1, n2)
